    ...
    const deleteUser = async () => {
        try {
            const response = await fetch(`http://localhost:3000/api/user-settings`, {
                ...
                body: JSON.stringify({ userId }),  // Send user ID to the server
            });

            if (response.ok) {
                console.log("User id succesfully deleted");
                await signOut();
                router.push("/");
            } else {
                        ...
                        <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                            <div className="sm:flex sm:items-start">
                                ...
                                <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                    ...
                                    <div className="mt-2">
                                        <p className="text-sm text-gray-500">Are you sure you want to <strong>Delete</strong> your account? All of your data will be permanently removed. This action cannot be undone.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                            <button onClick={deleteUser} type="button" className="inline-flex w-full justify-center rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-500 sm:ml-3 sm:w-auto">Delete</button>
