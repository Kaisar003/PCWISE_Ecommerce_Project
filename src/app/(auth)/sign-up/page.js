...
export default function SignUp() {
    ...
    async function handleSubmit(e) {
        ...
        try {
            ...
            setPending(false);
            setError("Something went wrong while registering", error);
        }
    }
    ...
    return (
        <>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
                ...
                <p className="text-xs font-light text-gray-500">By creating an account, you accept our <Link href="" className="font-medium text-link-color hover:underline">Terms of use</Link>. Find out how we handle your data on our website. <Link href="" className="font-medium text-link-color hover:underline">Privacy Policy</Link>.</p>
                {error && <span className="bg-red-500 text-white p-3 m-2 rounded-sm flex">{error}</span>}
                {pending ? <div role="status" className="flex justify-center">
                ...
                </div> : <button type="submit" className="w-full text-white bg-blue-card hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center duration-200">Register</button>}
